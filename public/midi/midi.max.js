module('users.timfelgentreff.midijs.midi').requires('users.timfelgentreff.midijs.libtimidity').toRun(() => {
    // Patch everything onto module
    const global = users.timfelgentreff.midijs.midi;
    const { Module } = users.timfelgentreff.midijs.libtimidity;
    ...
    let waveBuffer;
    let midiFileBuffer;
    let read_wave_bytes = 0;
    let song = 0;
    const midijs_url = new URL(module('users.timfelgentreff.midijs.midi').uri()).asDirectory().dirname();
    let start_time = 0;
    let audio_status = '';
    ...
    function get_next_wave(ev) {
        ...
        if (read_wave_bytes == 0) {
            stop_WebAudioAPI();
            return;
        }

        const max_i16 = Math.pow(2, 15);
